...
# Extract food insecurity data and calculate for sum scores- from followup data in 19, 21, 23, 25, 27, 31
  ...
  filter (Week %in% c(19, 21, 23, 25, 27, 31, 33, 35))%>%
...
# 0. Sample Disclaimer
  * This set of analyses on food insecurity is based on responses collected from 3,301 caregivers between the dates of August 11th 2020 and December 3rd 2020. These caregivers represent a range of voices: 6.88% are non-Latinx Black/African American, 16.09% are Latinx, and 27.59% live at or below 1.5 times the federal poverty line.  Proportions/percentages are calculated based on the item-level response rates, not out of the total sample size. The data for these analyses are not weighted. \
...
# 1. Prevalence of Food Insecurity
  ...
  * Note. \
    + Food insecurity data were collected on weeks 19, 21, 23, 25, 27, 31, 33, & 35 follow-up surveys. \
...
pmh_31fl <- pmh_fl %>%
  ...
  filter (Week_fl == min (Week_fl))
pmh_33fl <- pmh_fl %>%
  filter (CaregiverID %in% fi_33$CaregiverID & Week_fl > 33)%>%
  group_by (CaregiverID)%>%
  filter (Week_fl == min (Week_fl))
pmh_35fl <- pmh_fl %>%
  filter (CaregiverID %in% fi_35$CaregiverID & Week_fl > 35)%>%
  group_by (CaregiverID)%>%
  filter (Week_fl == min (Week_fl))
...
fi_pmh_33 <- merge (x = fi_33, y = pmh_31fl, by.x = c("CaregiverID", "baseline"), by.y = c("CaregiverID", "baseline"), all.x = T)
fi_pmh_35 <- merge (x = fi_35, y = pmh_31fl, by.x = c("CaregiverID", "baseline"), by.y = c("CaregiverID", "baseline"), all.x = T)

fi_pmh_fl <- rbind(fi_pmh_19, fi_pmh_21, fi_pmh_23, fi_pmh_25, fi_pmh_27, fi_pmh_31, fi_pmh_33, fi_pmh_35)
...
fi_cp_33 <- merge (x = fi_33, y = cp_31fl, by.x = c("CaregiverID", "baseline"), by.y = c("CaregiverID", "baseline"), all.x = T)
fi_cp_35 <- merge (x = fi_35, y = cp_31fl, by.x = c("CaregiverID", "baseline"), by.y = c("CaregiverID", "baseline"), all.x = T)

fi_cp_fl <- rbind(fi_cp_19, fi_cp_21, fi_cp_23, fi_cp_25, fi_cp_27, fi_cp_31, fi_cp_33, fi_cp_35)
  
```

## Mediation Analyses: Food Insecurity -> Caregivers' Mental Health -> Children's Mental Health
```{r, echo=FALSE, message = FALSE, warning = FALSE}
...
cp_31fl2 <- cp_fl2 %>%
  ...
  mutate (Week_fi = 31)
cp_33fl2 <- cp_fl2 %>%
  filter (CaregiverID %in% fi_33$CaregiverID & Week_fl2 > 33)%>%
  group_by (CaregiverID)%>%
  filter (Week_fl2 == nth(Week_fl2, 2, descending = F))%>%
  mutate (Week_fi = 33)
cp_35fl2 <- cp_fl2 %>%
  filter (CaregiverID %in% fi_35$CaregiverID & Week_fl2 > 35)%>%
  group_by (CaregiverID)%>%
  filter (Week_fl2 == nth(Week_fl2, 2, descending = F))%>%
  mutate (Week_fi = 35)

fi_cp_fl2 <- rbind(cp_19fl2, cp_21fl2, cp_23fl2, cp_25fl2,cp_27fl2, cp_31fl2, cp_33fl2, cp_35fl2 )

fi_cp_fl2 <- merge (x = fi_cp_fl, y = fi_cp_fl2, by.x = c("CaregiverID",  "Week_fi"), by.y = c("CaregiverID", "Week_fi"), all.x = T)%>%
  select("CaregiverID", "Week_fi", contains ("ctotal"))
...
### Path c (direct effect): food insecurity -> children's mental health problems at second subsequent response
  * Food insecurity significantly predicted increased total mental health problems among children during the second subsequent response(b = 1.36, p < .001), controlling for the current total mental health problems score (b = .62, p < .001)\
```{r, echo=FALSE, message = FALSE, warning = FALSE}
...
### Path a: food insecurity -> caregivers's mental health problems at first subsequent response
  * Food insecurity significantly predicted increased total mental health problems among children during the second subsequent response(b = .79, p < .001), controlling for the current total mental health problems score (b = .76, p < .001)\
```{r, echo=FALSE, message = FALSE, warning = FALSE}
...
### Path b: caregivers' mental health problems at first subsequent response -> children's mental health problems at second subsequent response
  * Caregivers' mental health problems at the first subsequent response significantly predicted increased total mental health problems among children during the second subsequent response(b = .15, p < .001), controlling for children's total mental health problems score at the first subsequent response (b = .62, p < .001) and current food insecurity (path c': b = .90, p < .001)\
```{r, echo=FALSE, message = FALSE, warning = FALSE}
...
### Testing Indirect Effect using RMediation Method
  * Indirect effect was statistically significant: a*b = .121, 95%CI[.08, .16], p <.001 
```{r, echo=FALSE, message = FALSE}
library (RMediation)
medci(mu.x = .7861, se.x = .1086, mu.y = .1533, se.y = .0147) 
