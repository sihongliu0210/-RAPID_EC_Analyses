...
covid <- master_fl %>%
  select (CaregiverID, StartDate, SurveyType, Week, month, baseline, contains ("COVID"), contains ("PHQ"), contains ("GAD"), STRESS.001, STRESS.002, LONE.001.a, LONE.001.b, contains("CBCL") )

covid$depress_crt= rowMeans(covid[,find_items("PHQ.002.[a-b]$", covid)], na.rm=T)
covid$anxiety_crt = rowMeans(covid[,find_items("GAD2.002.[a-b]$", covid)], na.rm=T)
...
fussy_crt_items = c(find_items("fussy_cr", covid))
fear_crt_items = c(find_items("fear_cr", covid))
covid$fussy_crt = rowMeans(covid[,fussy_crt_items], na.rm=T)/2*100
covid$fear_crt = rowMeans(covid[,fear_crt_items], na.rm=T)/2*100

cp_crt_items = c("fussy_crt", "fear_crt")
covid$cp_total_crt = rowMeans (covid[, cp_crt_items], na.rm=T)

pmh_crt_variable = c("depress_crt_r", "anxiety_crt_r", "stress_crt_r", "lonely_crt_r")
covid$pmh_total_crt = rowMeans(covid[, pmh_crt_variable], na.rm = T)

covid <- merge (x = master_dem_fl, y = covid, by.x = "CaregiverID", by.y = "CaregiverID", all.y = T)%>%
  select (CaregiverID, StartDate, SurveyType, Week, month, baseline, race_ethnic, poverty_cat, contains ("COVID"), pmh_total_crt, cp_total_crt)
...
covid_dg <- covid %>%
  ...
  ungroup()%>%
  mutate (suspect_or_diagnosis = ifelse(COVID.001 %in% c(1,2,3), 1, 0),
          diagnosis = ifelse(COVID.001 %in% c(2,3), 1, 0),
...
covid_dg_smy <- covid_dg %>%
  summarise (n = n(),
             ...
             c.no = sum(no, na.rm = TRUE)/n,
             b.suspect = sum(suspect, na.rm = TRUE)/n
             ) %>%
  pivot_longer (cols = c("a.diagnosis", "c.no", "b.suspect"), names_to = "condition", values_to = "prop") %>%
  mutate (count = n*prop) %>%
...
covid_trend <- covid %>%
  ...
  pivot_longer (cols = c("c.no", "b.suspect", "a.diagnosis"), names_to = "condition", values_to = "prop") %>%
  mutate (count = prop * n, 
...
# 1.3 Association with Mental Health - Based on Most Recent Responses
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_rct <- covid %>%
  group_by (CaregiverID)%>%
  filter (Week == max (Week))%>%
  ungroup()%>%
  mutate (covid_status = case_when (no == 1 ~ "Not infected",
                                    suspect == 1 ~ "Suspect", 
                                    diagnosis == 1 ~ "Diagnosed", 
                                    ))

covid_mh_m <- covid_rct %>%
  group_by (covid_status) %>%
  summarise (n = n(),
             parent = mean(pmh_total_crt, na.rm = TRUE), 
             child = mean(cp_total_crt, na.rm = TRUE) ) %>%
  pivot_longer (cols = c("parent", "child"), names_to = "who", values_to = "mean") 

covid_mh_sd <- covid_rct %>%
  group_by (covid_status) %>%
  summarise (parent = sd(pmh_total_crt, na.rm = TRUE), 
             child = sd(cp_total_crt, na.rm = TRUE) ) %>%
  pivot_longer (cols = c("parent", "child"), names_to = "who", values_to = "sd") 

covid_mh_smy <- merge (covid_mh_m, covid_mh_sd)%>%
  mutate (moe = map2_dbl(sd, n, moe.m), 
          ci_low = mean-moe,
          ci_high = mean+moe)

covid_mh_smy %>%
  filter (covid_status != "NA")%>%
  ggplot(mapping = aes (x = covid_status, y = mean, fill = covid_status)) +
  geom_bar (stat = "identity", position = position_dodge(1))+
...
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_know_dg <- covid %>%
  filter (Week >= 29 & SurveyType == 2 & race_ethnic!= "NA")%>%
  group_by (Week, race_ethnic)%>%
  summarise (n = n(),
             know_diagnose = sum(COVID.003, na.rm = TRUE)/n, 
             know_hospitalized_died = sum (COVID.004, na.rm = T)/n, 
...
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_know_dg <- covid %>%
  filter (Week >= 29 & SurveyType == 2 & poverty_cat!= "NA")%>%
  group_by (Week, poverty_cat)%>%
  summarise (n = n(),
             know_diagnose = sum(COVID.003, na.rm = TRUE)/n, 
             know_hospitalized_died = sum (COVID.004, na.rm = T)/n, 
...
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_know_dg <- covid %>%
  filter (Week >= 29 & SurveyType == 2 & poverty_cat!= "NA" & race_ethnic!= "NA")%>%
  group_by (Week, poverty_cat, race_ethnic)%>%
  summarise (n = n(),
             know_diagnose = sum(COVID.003, na.rm = TRUE)/n, 
             know_hospitalized_died = sum (COVID.004, na.rm = T)/n, 
...
# 2.3 Association with Mental Health - Based on Most Recent Responses
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_rct <- covid %>%
  filter (Week >= 29 & SurveyType == 2)%>%
  group_by (CaregiverID)%>%
  filter (Week == max (Week))%>%
  ungroup()%>%
  mutate (know_diagnose = case_when (COVID.003 == 1 ~ "Know someone diagnosed with COVID-19",
                                    COVID.003 == 0 ~ "Not know someone diagnosed with COVID-19"), 
          know_hospitalized_died = case_when (COVID.004 == 1 ~ "Know someone hospitalized/died from COVID-19",
                                    COVID.004 == 0 ~ "Not know someone hospitalized/died from COVID-19"))

covid_mh_m <- covid_rct %>%
  group_by (know_diagnose) %>%
  summarise (n = n(),
             parent = mean(pmh_total_crt, na.rm = TRUE), 
             child = mean(cp_total_crt, na.rm = TRUE) ) %>%
  pivot_longer (cols = c("parent", "child"), names_to = "who", values_to = "mean") 
...
covid_mh_smy %>%
  filter (know_diagnose != "NA")%>%
  ggplot(mapping = aes (x = know_diagnose, y = mean, fill = know_diagnose)) +
  geom_bar (stat = "identity", position = position_dodge(1))+
  geom_errorbar(aes(ymin = (mean-moe), 
                    ymax = (mean+moe)),
                 width = .3,
  ...
  facet_wrap (~who)+ 
  theme(axis.text.x = element_blank())+
  ggtitle (label = "Based on knowing someone diagnosed with COVID-19")
```

```{r, echo=FALSE, message = FALSE, warning = FALSE}

covid_mh_m <- covid_rct %>%
  group_by (know_hospitalized_died) %>%
  summarise (n = n(),
             parent = mean(pmh_total_crt, na.rm = TRUE), 
             child = mean(cp_total_crt, na.rm = TRUE) ) %>%
  pivot_longer (cols = c("parent", "child"), names_to = "who", values_to = "mean") 
...
covid_mh_smy %>%
  filter (know_hospitalized_died != "NA")%>%
  ggplot(mapping = aes (x = know_hospitalized_died, y = mean, fill = know_hospitalized_died)) +
  geom_bar (stat = "identity", position = position_dodge(1))+
  geom_errorbar(aes(ymin = (mean-moe), 
                    ymax = (mean+moe)),
                 width = .3,
  ...
  facet_wrap (~who)+ 
  theme(axis.text.x = element_blank())+
  ggtitle (label = "Based on knowing someone hospitalized/died from COVID-19")
```