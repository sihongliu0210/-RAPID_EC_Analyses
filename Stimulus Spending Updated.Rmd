...
```{r, echo=FALSE, message = FALSE, warning = FALSE}
stimulus12 <- stimulus_rct %>%
  mutate (food = ifelse (STIM.001.b_1 == 1|STIM.002.b_1 == 1, 1, 0),
          clothing = ifelse (STIM.001.b_2 == 1|STIM.002.b_2 == 1, 1, 0),
          supplies_personal_care = ifelse (STIM.001.b_3 == 1|STIM.002.b_3 == 1, 1, 0),
          household_items = ifelse (STIM.001.b_4 == 1|STIM.002.b_4 == 1, 1, 0),
          recreational = ifelse (STIM.001.b_5 == 1|STIM.002.b_1 == 5, 1, 0),
          rent = ifelse (STIM.001.b_6 == 1|STIM.002.b_6 == 1, 1, 0),
          mortgage = ifelse (STIM.001.b_7 == 1|STIM.002.b_7 == 1, 1, 0),
          utilities = ifelse (STIM.001.b_8 == 1|STIM.002.b_8 == 1, 1, 0),
          vehicle_pay = ifelse (STIM.001.b_9 == 1|STIM.002.b_9 == 1, 1, 0),
          credit_card_loans = ifelse (STIM.001.b_10 == 1|STIM.002.b_10 == 1, 1, 0),
          donations = ifelse (STIM.001.b_11 == 1|STIM.002.b_11 == 1, 1, 0),
          saving_investment = ifelse (STIM.001.b_12 == 1|STIM.002.b_12 == 1, 1, 0),
          other = ifelse (STIM.001.b_13 == 1|STIM.002.b_13 == 1, 1, 0))

stimulus_12_smy <- stimulus12 %>%
  filter (STIM.001==1|STIM.002==1)%>%
  mutate (basic_need = ifelse (food == 1 |rent == 1 |mortgage == 1 | utilities == 1  , 1, 0), 
          other_essentials = ifelse (vehicle_pay == 1 |credit_card_loans == 1|clothing == 1 | supplies_personal_care == 1|household_items == 1, 1, 0), 
          recreational_activities = ifelse (recreational == 1, 1, 0),
          saving_investment = ifelse (saving_investment == 1, 1, 0))%>%
  summarise (n = n(),
             basic_need = sum (basic_need, na.rm = T)/n, 
             other_essentials = sum (other_essentials, na.rm = T)/n,
             recreational_activities = sum (recreational_activities, na.rm = T)/n,
  ...
  mutate (count = prop * n, 
          ...
          ci_low = prop-moe,
          ci_high = prop+moe, 
          which = "first & second stimulus check")
...
stimulus_12_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -1.5, size = 3, color = "black",  position = position_dodge(1))+
  labs (y = "Percentage")+
  ggtitle("first & second stimulus check")+
  theme(legend.position = "none")
...
stimulus_12_smy %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle("first and second stimulus check")+
  scale_x_discrete(breaks=unemployment$FPL_pct_cn, labels=unemployment$FPL_pct_c)+
  facet_wrap(~stimulus, ncol = 5)+
  theme(legend.position = "none")
