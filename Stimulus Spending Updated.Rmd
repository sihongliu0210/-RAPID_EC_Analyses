...
master <- master %>%
  mutate (poverty_100 = FPL.100, 
          poverty_200 = FPL.200, 
          poverty_3c = case_when (FPL == 1 ~ 1,
                                  FPL %in% c(2,3,4)~ 2,
                                  FPL == 5 ~ 3))
...
## Overall - categorized into 4 groups
  * Four categories:
    ...
    + Other financial obligations: Vehicle payments, paying down credit card, student loans, or other debts\
    + Discretionary: clothing, household supplies and personal care, household items (TV, electronics, furniture, appliances), recreational goods, charitable donations or giving to family members, other\
...
stimulus_1 <- stimulus_rct %>%
  ...
  mutate (basic_need = ifelse (STIM.001.b_1 == 1 |STIM.001.b_6 == 1 |STIM.001.b_7 == 1 |STIM.001.b_8 == 1, 1, 0), 
          other_financial_obligation = ifelse (STIM.001.b_9 == 1 |STIM.001.b_10 == 1, 1, 0 ), 
          discretionary = ifelse (STIM.001.b_2 == 1 | STIM.001.b_3 == 1|STIM.001.b_4 == 1|STIM.001.b_5 == 1|STIM.001.b_11 == 1|STIM.001.b_13==1, 1, 0), 
          saving_investment = ifelse (STIM.001.b_12 == 1, 1, 0))%>%
  summarise (n = n(),
             basic_need = sum (basic_need, na.rm = T)/n, 
             other_financial_obligation = sum (other_financial_obligation, na.rm = T)/n, 
             discretionary = sum (discretionary, na.rm = T)/n,
             saving_investment = sum (saving_investment, na.rm = T)/n)%>%
  pivot_longer (cols = c("basic_need", "other_financial_obligation", "discretionary", "saving_investment"), names_to = "stimulus", values_to = "prop")%>%
  mutate (count = prop * n, 
          moe = map2_dbl(prop, n, moe.p), 
          ci_low = prop-moe,
          ci_high = prop+moe, 
...
stimulus_2 <- stimulus_rct %>%
  ...
  mutate (basic_need = ifelse (STIM.002.b_1 == 1 |STIM.002.b_6 == 1 |STIM.002.b_7 == 1 |STIM.002.b_8 == 1, 1, 0), 
          other_financial_obligation = ifelse (STIM.002.b_9 == 1 |STIM.002.b_10 == 1, 1, 0 ), 
          discretionary = ifelse (STIM.002.b_2 == 1 | STIM.002.b_3 == 1|STIM.002.b_4 == 1|STIM.002.b_5 == 1|STIM.002.b_11 == 1|STIM.002.b_13==1, 1, 0), 
          saving_investment = ifelse (STIM.002.b_12 == 1, 1, 0))%>%
  summarise (n = n(),
             basic_need = sum (basic_need, na.rm = T)/n, 
             other_financial_obligation = sum (other_financial_obligation, na.rm = T)/n, 
...
unemployment <- stimulus_rct %>%
  ...
  mutate (basic_need = ifelse (STIM.003.b_1 == 1 |STIM.003.b_6 == 1 |STIM.003.b_7 == 1 |STIM.003.b_8 == 1, 1, 0), 
          other_financial_obligation = ifelse (STIM.003.b_9 == 1 |STIM.003.b_10 == 1, 1, 0 ), 
          discretionary = ifelse (STIM.003.b_2 == 1 | STIM.003.b_3 == 1|STIM.003.b_4 == 1|STIM.003.b_5 == 1|STIM.003.b_11 == 1|STIM.003.b_13==1, 1, 0), 
          saving_investment = ifelse (STIM.003.b_12 == 1, 1, 0))%>%
  summarise (n = n(),
             basic_need = sum (basic_need, na.rm = T)/n, 
             other_financial_obligation = sum (other_financial_obligation, na.rm = T)/n, 
