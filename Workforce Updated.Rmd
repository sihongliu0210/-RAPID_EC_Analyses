...
wkf$WKFORCE.001.a_0[is.na(wkf$WKFORCE.001.a_0)] <- 0
wkf_crt <- wkf_rct %>%
...
wkf_done <- wkf %>%
  ...
  filter (is.na(WKFORCE.009) == F | is.na(010) == F)%>%
  summarise(n = n(),
            afford_reducing_hours = sum (WKFORCE.009, na.rm = T)/n,
            afford_stop_working = sum (WKFORCE.010, na.rm = T)/n)%>%
  pivot_longer (cols = c("afford_reducing_hours", "afford_stop_working"), names_to = "wkf", values_to = "prop")%>%
  mutate (count = prop * n, 
          moe = map2_dbl(prop, n, moe.p), 
          ci_low = prop-moe,
          ci_high = prop+moe)
